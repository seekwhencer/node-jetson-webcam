...
export default class extends Module {
    ...
    run() {
        ...
        const gstreamerOptions = {
            ...
            tcpmjpegnv: [
                ...
                '!', 'nvvidconv',
                '!', `video/x-raw(memory:NVMM),format=I420,width=${this.options.width},height=${this.options.height}`,

                '!', 'nvv4l2h264enc', 'preset-level=4', 'maxperf-enable=1', 'iframeinterval=100', 'insert-sps-pps=1',
                '!', `video/x-h264,stream-format=byte-stream`,

                '!', 'h264parse',
                '!', 'mpegtsmux',

                '!', 'tcpserversink', `port=${this.options.port}`, 'host=0.0.0.0', 'buffers-min=50000',
            ],
            /*
                        stream: [
                            '-v',
    ...
    record() {
        ...
        const options = [
            '-i',
            'tcp://127.0.0.1:5100', '-vcodec', 'copy',
            filePath,
        ];
        LOG(this.label, this.options.name, 'RECORDING...', this.options.bin, 'WITH OPTIONS', options.join(' '));

        this.processRecord = spawn(this.options.ffmpeg_bin, options);
